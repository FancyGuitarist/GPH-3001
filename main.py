...
def main():
    parser = argparse.ArgumentParser(prog="amt", description="Automatic Music transcription & Identification for musicians.", epilog="Music is the arithmetic of sounds as optics is the geometry of light. -Claude Debussy", formatter_class=CapitalizedHelpFormatter)
    ...
    for p in [mono, poly, chord]:
        p.add_argument('-pr', '--piano-roll', action='store_true', help='Show piano roll visualization')
        ...
        if p == poly:
            ...
            p.add_argument('-t','--threshold', type=float, help='Threshold for the detection of note in with respect to 1', metavar='[0-1]')
    ...
    elif args.Modes == "chord_only":
        simple_notation = []
        print("chord-Only mode")

    else:
        print("Monophonic mode (default)")
        mono = Mono(audio)
