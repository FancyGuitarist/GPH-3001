...
class Pseudo2D(AudioParams):
    ...
    def cross_correlate_diag(self, pseudo2dSpectrum, _2D=True):
        ...
        gamma = 6_700  # 6_7k sweet spot pareille.
    ...
    def best_estimate(self, cross_corr: np.ndarray):
        """
        gives best estimate of the pitch by returning the index of the cross correlation that is above the threshold
        """
    ...
    def filter_short_notes(self, piano_roll: np.ndarray, min_length=3):
        """
        ...
        for note in range(piano_roll.shape[0]):
            ...
            for i in range(length):
                ...
                # if (i < length-1) & (note_onsets[i + 1] - note_offsets[i] < min_length):
                #     piano_roll[note, note_offsets[i]:note_onsets[i + 1]] = 1
        piano_roll = np.roll(piano_roll, -2, axis=1)
    ...
    def show_multipitch_estimate(self):
        _, piano = self.multipitch_estimate()
